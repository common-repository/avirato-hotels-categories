...
function ahc_open_tab(evt, tabName) {
    // Declare all variables
    var i, ahc_tabcontent, ahc_tablinks;

    // Get all elements with class="tabcontent" and hide them
    ahc_tabcontent = document.getElementsByClassName("ahc_tabcontent");
    for (i = 0; i < ahc_tabcontent.length; i++) {
        ahc_tabcontent[i].style.display = "none";
    }

    // Get all elements with class="tablinks" and remove the class "active"
    ahc_tablinks = document.getElementsByClassName("ahc_tablinks");
    for (i = 0; i < ahc_tablinks.length; i++) {
        ahc_tablinks[i].className = ahc_tablinks[i].className.replace(" active", "");
    }
...
//        jQuery('#acip_alertInput').css('opacity', '0');
    ...
    jQuery('#ahc_noShow').html(htmlMultiOP);
